...
public class CoffeeController {
    ...
    ////修改查询方法，指定返回格式为JSON
    @GetMapping(value = "/json/{id}",produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public Coffee getJSONById(@PathVariable Long id) {
        Coffee coffee = coffeeService.getCoffee(id);
        log.info("Coffee {}:", coffee);
        return coffee;
    }
    ...
    public Coffee getXMLByName(@RequestParam String name) {
        Coffee coffee = coffeeService.getCoffee(name);
        log.info("Coffee xml {}:", coffee);
        return coffee;
    }
